"""
Hash-delta helpers for the Deepideas gap-ingredients export.
...
ingredient rows share a keyhash; _rowhash carries the ingredient grain.
Delta still works because the send filter is
  keyhash new OR (keyhash known AND rowhash new).

Staging table names follow the Composer loop variable
`gap_ingredients` → di_gap_ingredients_export_{today|yesterday}.
...
YESTERDAY = "staging.di_gap_ingredients_export_yesterday"
...
SELECT
  ...
  CAST(product_main_cat_id AS STRING) AS product_main_cat_id,
