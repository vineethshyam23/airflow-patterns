"""
Avro bulk ingest for Deepideas gap-ingredient rows.
...
Sanitized fixes vs production module:
  ...
  - wholesale_id replaces metro_id; product_main_cat_id replaces mge_*
...
if ENV == "DEV":
    ...
    SCHEMA_ID = Variable.get(
        "gap_ingredients_schema_id_dev",
        default_var="GAP_INGREDIENTS_SCHEMA_DEV",
    )
    SCHEMA_NAME = "foodgraph_gap_ingredients_dev"
    ...
    SCHEMA_ID = Variable.get(
        "gap_ingredients_schema_id",
        default_var="GAP_INGREDIENTS_SCHEMA_PROD",
    )
    SCHEMA_NAME = "foodgraph_gap_ingredients"
    INGEST_BASE = Variable.get(
...
AVRO_SCHEMA = json.dumps(
    {
        ...
        "name": SCHEMA_NAME,
        "doc": (
            "Menu-implied ingredients with no wholesale purchase "
            "revenue in the last year (ingredient-level gaps)"
        ),
        ...
        "fields": [
            {"name": "wholesale_id", "type": "long", "doc": "Wholesale customer id"},
            {
                ...
                "type": "string",
                "doc": "Name of gap ingredient",
            },
            {
                "name": "product_main_cat_id",
                "type": "string",
                "doc": "Id of gap product main category for the ingredient",
            },
            {
                "name": "relevance",
                "type": "string",
                "doc": (
                    "Ingredient score from menu prioritization "
                    "(position in top-seller frequency); higher is stronger"
                ),
            },
        ],
    }
...
def send_gaps_ingredients_data(country: str, query: str) -> None:
    ...
    log.info("%s - Executing gap ingredients query...", datetime.now())
    ...
    log.info(
        "%s - Gap ingredients export complete (%s rows).",
        datetime.now(),
        len(encoded),
    )
