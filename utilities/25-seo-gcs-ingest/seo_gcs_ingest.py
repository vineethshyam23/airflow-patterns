"""SEO business-listing GCS ingest helpers for Airflow.

Bucket layout (e.g. gs://dwh-seo-business-listing/):
...
Public entry points (called from etl_seo_business_listing_ingest DAG):
...
# Each line is one establishment; see schema_seo_business_listing.json.
# Example row (abbreviated):
#   title: "Black Duck Coffee & Bar"
...
SAMPLE_ROW: dict = {
    "title": "Black Duck Coffee & Bar",
    "address_info": {
        "borough": "Hohenlimburg",
        "address": "Ob. Isenbergstraße 1",
        "city": "Hagen",
        "zip": "58119",
        "region": None,
        "country_code": "DE",
...
def _iter_decompressed_lines(blob: storage.Blob, compression: str) -> Iterator[bytes]:
    ...
    Example:
        ...
        [b'{"title": "Black Duck Coffee & Bar", "address_info": {"country_code": "DE"}, ...}\\n']
    """
...
def _extract_country(row: dict) -> Optional[str]:
    """Extract ISO country code from one SEO vendor business listing row.
    ...
    Example:
        ...
        'DE'
        >>> _extract_country({"title": "Black Duck Coffee & Bar"})
        None
    """
...
def _parse_row_timestamp(value: str) -> Optional[datetime]:
    """Parse SEO vendor timestamp strings such as ``2025-11-15T14:22:57``."""
    value = value.strip()
...
def _max_timestamp(existing: Optional[str], candidates: Iterable[str]) -> Optional[str]:
    """Return the latest SEO vendor timestamp string on a row.
...
def _min_timestamp(existing: Optional[str], candidates: Iterable[str]) -> Optional[str]:
    """Return the earliest SEO vendor timestamp string on a row.
...
def archive_all_stg_to_load(
    ...
    *,
    ...
    Args:
        bucket_name: Ingest bucket (e.g. ``dwh-seo-business-listing``).
    ...
    Example:
        >>> archive_all_stg_to_load("dwh-seo-business-listing").archived
...
def _add_ingest_args(parser: argparse.ArgumentParser) -> None:
    ...
    parser.add_argument(
        "--bucket",
        required=True,
        help="Ingest bucket name (e.g. dwh-seo-business-listing)",
    )
    parser.add_argument(
        "--upload-date",
        metavar="YYYYMMDD",
...
def main(argv: Optional[list[str]] = None) -> int:
    ...
    parser = argparse.ArgumentParser(
        description="SEO vendor GCS ingest — scan, ingest, archive",
